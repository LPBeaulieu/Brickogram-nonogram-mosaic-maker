...
- You will need to have all the scans of the differently colored 1 x 1 plate that will make up your mosaic present within the "1 x 1 plate scans" folder in JPEG format, with the file names starting with the parenthesized RGB values, followed by the color name (ex: "(0, 85, 191) Blue.jpg"). This will allow the Python code to generate a thumbnail image of the mosaic and to include a solution key in the PDF documents. A procedure is included in the PDF document guiding you on how to add more colors to the "1 x 1 plate scans" folder. 
...
- There are a few arguments that you could pass in after the above command, should you like to get different outputs:
  ...
  - The code automatically picks the lightest color in each nonogram grid making up the mosaic, insofar as the number of 1 x 1 plates of that color within the grid is above the threshold (default of 5% of the total number of 1 x 1 plates in the grid, so 52 or more plates for a 32 x 32 grid). Should you want to specify a different threshold, simply pass in the desired percentage after the "auto_blanks:" argument (ex: "auto_blanks:10" for a 10% threshold).
    ...
    Alternatively, should you wish to only have blanks of a single color for all of the nonograms generated for the images found in the "Images" folder, enter the color after the “blank:” argument, either in hex code or RGB format. The RGB colors need to be parenthesized (' py brickogram.py "blank:(255,255,255)" ' would select white as the blank for all the nonogram puzzles making up each pixelated image in the "Images" folder).
