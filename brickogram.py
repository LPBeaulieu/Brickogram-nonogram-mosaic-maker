...
img_files = [file for file in img_files if file[-4:].lower() != ".pdf"]
...
#value of the blank that will be used thoughout a given
...
#automatically selects the lightest color in each pixelated
#image as the blank. Otherwise, you can select a blank color
#that would apply to all the nonogram puzzles generated for
#the pixelated images found within the "Images" folder by
#providing the blank color hex code or parenthesized RGB
#value after the "blank:" argument. For example, entering
...
top_string = "Top"
side_string = "Side"
side_panel_pdf_name = "Brickograms Side Panels.pdf"
top_panel_pdf_name = "Brickograms Top Panels.pdf"
...
with alive_bar(len(img_files)) as bar:
    for i in range(len(img_files)):
        ...
        for z in range(1):
            ...
            #If you haven't provided any color values to be used as the blank after the "blank:" argument
            #when running the code, then the value of "blank" is still equal to "None" at this point. Provided
            #that you haven't passed in the "no_blanks" argument indicating that you wish all of the color clues
            #to be present on the nonogram clue sheets, the code will procede to find which is the lightest color
            #in the pixelated image. It does so by calculateing the grayscale equivalent for the RGB values of
            #the colors present in the "colors" dictionary (k[0]+k[1]+k[2])/3, where k represents the color RGB key
            #in the "colors" dictionary). Should there be two or more lightest colors sharing a same grayscale value,
            #the most common one will be chosen as the blank (the one with the highest value (or color counter) in the
            #"colors" dictionary.)
            if not blank and not no_blanks:
                colors_grayscale = sorted([[math.floor((k[0]+k[1]+k[2])/3), v, k] for k, v in colors.items()])
                #The "key = lambda x:x[1]" sorts the list comprehension results along the first element,
                #which corresponds to the number of aggregated pixels of a given color (v).
                minimum_entries = sorted([element for element in colors_grayscale if element[0] == colors_grayscale[-1][0]], key = lambda x:x[1])
                blank_tuple = minimum_entries[-1][2]
            ...
            if problematic_rows or problematic_columns:
                ...
                if problematic_rows:
                    print('\nThe following rows (given as the "y" indices of the aggregated pixels in the image, ' +
                    'followed by the baseplate row numbers and the "y" indices in those baseplates) have a number of color ' +
                    ...
                    for j in range(len(problematic_rows)):
                        print("\n- Image aggregated pixel row number " + str(problematic_rows[j][0]) +
                        " (baseplate row number " + str(problematic_rows[j][1]) + ", row number " +
                        str(problematic_rows[j][2]) + ", baseplate column number " +
                        str(problematic_rows[j][3]) + ") has a total of " + str(problematic_rows[j][4]) +
                        " color transitions.")

                if problematic_columns:
                    print('\nThe following columns (given as the "x" indices of the aggregated pixels in the image, ' +
                    'followed by the baseplate column numbers and the "x" indices in those baseplates) have a number of color ' +
                    'transitions (excluding the blanks) above the maximal amount of ' + str(color_transition_threshold) +
                    ' that fit on a page.')
            ...
            if i == 0:
                ...
                background_img.save(os.path.join("Images", side_panel_pdf_name),
                ...
                background_img.save(os.path.join("Images", top_panel_pdf_name),
                quality=100, resolution=300)
                ...
                #the "title_page_text_font_size".
                title_page_text_strings = [[len(title_page_materials_string), title_page_materials_string],
                [len(title_page_blank_string), title_page_blank_string], [len(title_page_no_blanks_string),
                title_page_no_blanks_string]]
                ...
                #Here a verification is made as to whether the "title_page_blank_string" is not
                #in fact the longest string on the title page when factoring in the space taken
                #by the 1x1 plate that is pasted onto the image after it. A separate "if" statement
                #is used here as the measurements of the blank string needs to be determined in
                #order to paste the 1x1 plate later on in the code, whether or not string is the
                #longest one on the title page.
                if blank:
                ...
                title_page_blank_string_check = [element for element in title_page_text_strings if element[1] == title_page_blank_string]
                if title_page_blank_string_length + 150 > title_page_text_strings[-1][0]:
            ...
            if no_blanks == True:
                background_img_editable.text((perforations_margin, y), title_page_no_blank_string,
            ...
            #The "y" coordinate is incremented by the equivalent of 0.75 inch below the
            ...
            for j in range(len(colors_sorted)):
                ...
                #"title_page_no_blanks_string" or "In this puzzle,
                #the blank color is:" string) is used to increment the "y"
            ...
            #"same_color_side[0:0+32]". Similarly, the "for l in range(math.ceil(width/
            ...
            for j in range(math.ceil(len(same_color_side)/nonogram_cells)):
                ...
                for l in range(math.ceil(width/pixel_size/nonogram_cells)):
                    starting_y = margin + non_printable_area_margin
                    for m in range(len(current_array)):
                    ...
                    "  " + str(j+1) + ", "  + column_string + " " + str(l+1) + ")")

                    background_img_editable.text((math.floor(background_img_width/2),
                    ...
                    background_img.rotate(180).save(os.path.join("Images", side_panel_pdf_name),
                    append=True, quality=100, resolution=300)
            ...
            for j in range(math.ceil(len(same_color_top)/nonogram_cells)):
                ...
                for l in range(math.ceil(width/pixel_size/nonogram_cells)):
                    ...
                    "  " + str(l+1) + ", "  + column_string + " " + str(j+1) + ")")

                    background_img_rotated_editable.text((math.floor(background_img_height/2),
                    non_printable_area_margin + A4_margin), puzzle_text, fill="Black",
                    font=title_font, anchor="mt")
                    ...
                    #a column, instead of down a row. The "l" index mapping to the current "column chunk"
                    ...
                    top_image_objects.append([l, j, background_img_rotated])
            ...
            for j in range(len(top_image_objects)):
                #The final image is outputted in PDF format.
                top_image_objects[j][2].save(os.path.join("Images", top_panel_pdf_name),
                append=True, quality=100, resolution=300)
